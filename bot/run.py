...
async def handle_message(message: types.Message):
    ...
    if message.chat.type == "private":
        await redisStreamRequest(message)
    if is_mentioned_in_group_or_supergroup(message):
        if message.text is not None:
            ...
            prompt = text_without_mention
        await redisStreamRequest(message, prompt)
...
async def redisStreamRequest(message: types.Message, prompt: str = None):
    try:
        ...
        logging.info(
            f"[RedisStream]: Pushing '{prompt}' to raw_input stream for {message.from_user.first_name} {message.from_user.last_name}"
        )
        payload = ACTIVE_CHATS.get(message.from_user.id)
        await redis_client.xadd("raw_input", payload)

    except Exception as e:
        print(f"-----\n[RedisStream-ERR] CAUGHT FAULT!\n{traceback.format_exc()}\n-----")
        await bot.send_message(
            chat_id=message.chat.id,
...
async def listen_processed_output():
    while True:
        try:
            response = await redis_client.xread(["processed_output"], block=0)
            if response:
                for stream, messages in response:
                    for message_id, message_data in messages:
                        user_id = message_data["user_id"]
                        response_text = message_data["response"]
                        await send_response(user_id, response_text)
        except Exception as e:
            print(f"-----\n[RedisStream-ERR] CAUGHT FAULT!\n{traceback.format_exc()}\n-----")
...
async def main():
    ...
    asyncio.create_task(listen_processed_output())
